...
if __name__ == "__main__":
    ...
    σ_norm = lambda z: (math.sqrt(1+ϵ * z**2) - 1) / ϵ      # Nonnegative map
    σ_norm_vec = lambda z: (math.sqrt(1 + ϵ * np.linalg.norm(z, axis=None)**2) - 1) / ϵ
    σ_norm_single_vec = lambda z: (math.sqrt(1 + ϵ * np.linalg.norm(z)**2) - 1) / ϵ

    """
    GRADIENT FUNCTION (derivative) of (8)
    σ_ϵ in (9)
    """
    σ_ϵ = lambda z: z / math.sqrt(1+ϵ *z**2)
    ...
    σ1 = lambda z: z/math.sqrt(1+z**2)                      # Uneven Sigmoidal function
    ...
    def neighbors(in_i, in_q, in_r=r):
        ...
        for j in range(N):
            if in_i == j:
                continue
            elif math.sqrt(np.sum((sim.data.get_body_xpos("quadrotor"+str(j)) - in_q)**2)) < in_r:
                Ns.append(j)
        return Ns
    ...
    #     import pdb; pdb.set_trace()
        ...
        zq.append(q_j - in_q)
        zp.append(p_j - in_p)
        aij = a_ij_vec(zq)                  # Spatial adjacency matrix
        sij = s_ij_vec(zq)                  # Stress elements associated with each edge (i,j) of proximity net G(q)
    #     import pdb; pdb.set_trace()
        term1 = np.transpose(sij* np.asarray(zq).transpose())
        term2 = np.transpose(aij * np.asarray(zp).transpose())
        return np.sum(term1 + term2, axis=1)   # See Paragraph around Eq 43
    ...
    while (True):
        ...
        for i in range(N):
            if (not first):
                ...
                print(u)
                print('u end')
                # TODO: Investigate why this does not work haha. u is the acceleration input...
                # really unsure whether it corresponds to xyz and if those values are correct
                Quadrotors[i].control(np.array([u[0][0]*dt**2,u[0][1]*dt**2,u[0][2]*dt**2]),       # xyz pose Desired
                            np.array([0, 0, 0, 0]),                                      # Rotation Quaternion Desired
                            np.array([0, 0 , 0]),                                        # Roll Pitch Yaw Desired
                            (np.array([u[0][0]*dt,u[0][1]*dt,u[0][2]*dt]), np.array([0,0,0])),                      # Velocity Desired
                            (u[0], np.array([0,0,0]))                       # Acceleration Desired
                )
            else:
